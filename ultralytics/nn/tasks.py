...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            # BiFPN_Add outputs same channels as first input (they should all match)
            c2 = ch[f[0]] if isinstance(f, list) else ch[f]
            args = [c2, len(f) if isinstance(f, list) else 1]
        elif m is ECAAttention:
            # ECAAttention doesn't change channel dimension
            c1 = ch[f]
            args = [c1]
        elif m in frozenset(
            {
