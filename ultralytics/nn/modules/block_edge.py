...
from .conv import Conv, DWConv, GhostConv, LightConv, autopad
...
class RepConv_Edge(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, k: int = 3, s: int = 1, p: int = None, g: int = 1, 
                 d: int = 1, act: bool = True, deploy: bool = False):
    ...
    def switch_to_deploy(self):
        ...
        self.rbr_reparam = nn.Conv2d(
            w_fused.size(1), w_fused.size(0), w_fused.size(2), 
            self.rbr_3x3.conv.stride, self.rbr_3x3.conv.padding,
            groups=self.rbr_3x3.conv.groups, bias=True
        )
...
class RepGhostBlock(nn.Module):
    ...
    Training: Ghost bottleneck structure
    Inference: Fused single path
    """
...
class C2f_Edge(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, e: float = 0.5):
        ...
        self.m = nn.ModuleList(
            EdgeBottleneck(self.c, self.c, shortcut) for _ in range(n)
        )
...
class C3_Edge(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, e: float = 0.5, 
        ...
        self.m = nn.Sequential(*(
            EdgeBottleneck(c_, c_, shortcut) for _ in range(n)
        ))
