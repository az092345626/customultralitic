...
class Detect_Edge(Detect):
    ...
    Replaces standard convolutions with depthwise separable for reduced FLOPs
...
class Detect_Edge_End2End(Detect_Edge):
    ...
    def forward(self, x: list[torch.Tensor]) -> torch.Tensor | dict:
        ...
        one2one_x = [torch.cat((self.one2one_cv2[i](xi), self.one2one_cv3[i](xi)), 1) 
        ...
        return {
            "one2one": y,
            "one2many": torch.cat(one2many_x, 1) if hasattr(self, 'cv2') else y,
            "feats": x
        }
