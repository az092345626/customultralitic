...
class ModelBenchmark:
    ...
    def benchmark_throughput(self, batch_sizes=None, duration=5):
    ...
    def export_and_benchmark(self, formats=None, int8=False):
...
def main():
    ...
    if len(all_results) > 1:
        ...
        best_map = max(
            all_results.items(), key=lambda x: x[1].get("accuracy", {}).get("mAP50-95", 0) if "accuracy" in x[1] else 0
        )
        ...
        if "accuracy" in best_map[1]:
            print(
                f"🎯 Most Accurate: {best_map[0]} (mAP50-95: {best_map[1].get('accuracy', {}).get('mAP50-95', 0):.4f})"
            )
