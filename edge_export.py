...
from ultralytics.nn.modules import RepConv_Edge, RepGhostBlock
...
def main():
    ...
    if not args.no_fuse or not args.no_reparam:
        model = optimize_for_edge(model, fuse_bn=not args.no_fuse, reparameterize=not args.no_reparam)
    ...
    if args.format == "engine" or args.format == "all":
        f = export_tensorrt(model, args.imgsz, args.half, args.int8, args.workspace, args.calib_images)
        exported_files.append(f)
